...
\begin{exercise}
Determine the spectrum of the following rings. For each case, describe:
\begin{itemize}
    \item The underlying set of prime ideals.
...
\item $\bZ/42\bZ$. 
\item $\bR[x]$.
\item $\bC[x,y]/(x^2+y^2-1)$. (Hint: Notice that $x^2+y^2=(x+iy)(x-iy)$ and the isomorphic rings have the same spectrum.)
\item $\bC[x,y]/(x^2,xy)$. Then compute the stalk of the structure sheaf $\mathcal{O}_{(0,0)}$ at the origin.
\end{enumerate}
