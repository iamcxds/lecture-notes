...
As we mentioned earlier, we want to give natural space structure for collection of mathematics objects. Now this goal can be achieved by definite a sheaf which characterize the ``families of objects $A$ over $\Sigma$" $\Bun_{A}(\Sigma)$, with additional symmetry by using action groupoid, which should be thought as moduli space $\mathcal{M}_A$ containing $A$, i.e. $\Map(\Sigma,\mathcal{M}_A) :=\Bun_{A}(\Sigma)$ 

\begin{example}[Moduli Space of Triangles]
  If we classify triangles up to similarity, then we can define a smooth stack $\cM_{\Delta}: \Cart^\op \to \mathbf{Grpd}$ as the families of triangles: 
  ...
  We can extend this to general smooth set $ \cM_{\Delta}(M):= \Hom_{\mathrm{Stk}(\Cart)}(M,\cM_\Delta)$. Concretely, for manifold $M$, taking a good covering $\{U_i\}_{i\in I}$ of $M$, by the sheaf condition, we have: 
  \[
...
\begin{example}[Moduli Space of Vector Bundles]
  ...
  \]
  For general manifold $M$, we find $ \cM_{Vect,k}(M)= \Hom_{\mathrm{Stk}(\Cart)}(M,\cM_{Vect,k})$ classifies the isomorphic groupoid of rank $k$ vector bundles $ E \to M$: taking a good covering $\{U_i\}_{i\in I}$ of $M$,
...
\begin{example}[Configuation Space of Points/Hilbert Scheme]
  For a manifold $M$, we can consider the compact configuration space of $d$ unordered points $\mathrm{Conf}^d_M$. This can be defined as a smooth stack $M^d \sslash S_d$, which is sheafification of prestack.

  \[
    \mathrm{Conf}^d_{M,pre}(\bR^m):=\{ (f_i) \in \Map_{sm}(\bR^m, M^d)\} \sslash S_d
  \]
  We can see  $ \mathrm{Sym}^d_M = (M^d \setminus \mathrm{diag} )/S_d\subset \mathrm{Conf}^dM$ as subspace of non-stacky points.
...
Recall for sheaf $\cF,\cG \in \Sh(\cC)$ we can define the \emph{inner hom} $[\cF,\cG]\in \Sh(\cC)$ to be again a sheaf, where $[\cF,\cG](C):=\Hom_{\PSh(\cC)}(\cF\times \yo(C),\cG)$.

A physics field theory contain following thing: 
\begin{enumerate}
  \item a spacetime manifold $\Sigma$ of dimensional $d$,
  \item a smooth sheaf (stack) of fields configuration $\cF$,
  \item an action functional $S: [\Sigma,\cF]\to \bR $ or $U(1)$, where $[\Sigma,\cF]$ is the field history,
  \item[(4.)] 
   usually the action is defined from Lagrangian $\mathcal{L}:\cF|_\Sigma \to \Omega^d|_\Sigma \in \Sh(\Sigma)$, and 
\[
...
\begin{example}[Vector Bundle]
  ...
  \right\}
  \]
  As we saw earlier, $E$ can be classified by a morphism of stack $ f_E: \Sigma \to \mathrm{B}\GL_k $, and we have universal vector bundle $p_k: \mathcal{U}_k \to \mathrm{B}\GL_k$, such that $E$ is the pullback $ f_E^* \mathcal{U}_k$. That is to say, the setion space can be also seen as collection of commutive diagram:
  \[
\Gamma(\Sigma,E)=
    % https://q.uiver.app/#q=WzAsMyxbMCwwLCJcXFNpZ21hIl0sWzEsMCwiXFxtYXRoY2Fse1V9X2siXSxbMSwxLCJcXG1hdGhybXtCfVxcR0xfayJdLFswLDFdLFswLDIsImZfRSIsMl0sWzEsMiwicF9rIl1d
    \left\{\begin{tikzcd}[ampersand replacement=\&,cramped]
	\Sigma \& {\mathcal{U}_k} \\
	\& {\mathrm{B}\GL_k}
	\arrow[from=1-1, to=1-2]
	\arrow["{f_E}"', from=1-1, to=2-2]
	\arrow["{p_k}", from=1-2, to=2-2]
\end{tikzcd} \right\}
  \]
  To define the Lagrangian $\mathcal{L}: \Gamma(-,E) \to \Omega^d$, we can define $ \mathcal{L}_i: [U_i,\bR^k] \to \Omega^d(U_i)$ such that $ g_{ij}^*\mathcal{L}_j=\mathcal{L}_i$. This is in general difficult to construct, since $g_{ij}^*$ can be very complicated. For example $\partial_\mu: [U_i,\bR^k] \to C^\infty(U_i), \phi \mapsto \partial_\mu \phi$, $g^*\partial_\mu \phi= \partial_\mu (g\phi)= (\mu, g \mathrm{d}\phi)+ (\partial_\mu g)\phi $. The introducing of gauge field can help to define such morphism.
\end{example}
...
We begin with a gauge linear sigma model(GLSM). We can consider sigma model of following target space $\mathbb{CP}^n:=\{(z_{\mu}) \in \bC^{n+1} | \sum_{\mu=0}^n |z_\mu|^2=1 \}/ U(1) $. This identification can be seen as quotient of manifold, we can also interpret as sheaf, let ${U_i\to \Sigma}_{i\in I}$ be a good covering:

\[
\mathbb{CP}^n(\Sigma)=\Lim \mathbb{CP}^n(U_\bullet)=\left\{ (\boldsymbol{\phi}_i)\in \prod_{i\in I} \Map(U_i,\{\sum_{\mu=0}^n |z_\mu|^2=1\}) \;\middle|\; \boldsymbol{\phi}_i|_{U_{ij}}=g_{ij} \cdot \boldsymbol{\phi}_j|_{U_{ij}}, g_{ij}\in U(1)(U_{ij})\right\}
\]
...
Explicitly, let $\Sigma$ be a manifold, and a good covering $ \{U_i\to \Sigma \}_{i\in I}$, then by definition 
  ...
  \[
    \mathrm{Obj}(\mathrm{B}U(1)_{conn}(\Sigma)) := \left\{ (A_i)\in \prod_{i\in I}\Omega^1(U_{i}),  g_{ij}\in C^\infty(U_{ij}, U(1)) \,\middle\vert\,g_{ij}g_{jk}|_{U_{ijk}}=g_{ik}|_{U_{ijk}},\frac{\mathrm{d}g_{ij}}{g_{ij}}+ A_i|_{U_{ij}}=  A_j|_{U_{ij}}\right\}
  \] 
  \[
...
We can have two morphism of stack $p:\mathrm{B}U(1)_{conn}\to\mathrm{B}U(1)$ which forget the connection $A_i$, and curvature $ F: \mathrm{B}U(1)_{conn} \to \Omega^2_{cl} $ by taking $\mathrm{d}A_i$. Moreover, by composing with $U(1)\cong \bR/\bZ \to \mathrm{B}\bZ $, we can get a further morphism Chern class $ \mathbf{c}:\mathrm{B}U(1)_{conn}\to\mathrm{B}U(1)\to \mathrm{B}^2\bZ$. In fact $\pi_0\mathrm{B}^2\bZ(M)=\mathrm{H}^2(M,\bZ)$, thus $ \pi_0\mathbf{c}: \pi_0\mathrm{B}U(1)_{conn}(M)\to \mathrm{H}^2(M,\bZ)$ is the usual Chern class.

In fact $ \mathrm{B}U(1)_{conn}$ have both geometric and topological information: the $F$ extract the geometric data, gives the curvature form and $\mathbf{c}$ gives the topological twist of the bundle. We can link those two by the ${\mathrm{B}^2\bR_{disc}}$ which classify real coefficient cohomology group $ \mathrm{H}^2(M,\bR)$:
% https://q.uiver.app/#q=
\[\begin{tikzcd}[ampersand replacement=\&,cramped]
	\& {\Omega^2_{cl}} \\
	{\mathrm{B}U(1)_{conn}} \&\& {\mathrm{B}^2\bR_{disc}} \\
	\& {\mathrm{B}^2\bZ}
	\arrow["{\text{De Rham}}", from=1-2, to=2-3]
	\arrow["F", from=2-1, to=1-2]
	\arrow["\lrcorner"{anchor=center, pos=0.125, rotate=45}, draw=none, from=2-1, to=2-3]
	\arrow["{\mathbf{c}}"', from=2-1, to=3-2]
	\arrow[from=3-2, to=2-3]
\end{tikzcd}\]
We even get a pullback diagram, which means $ \mathrm{B}U(1)_{conn}$ is gluing of geometric data $ \Omega^2_{cl}$ and topological data $\mathrm{B}^2\bZ $. That is why it is also called \emph{differential cohomology}.
...
Let $\mathcal{G},\mathcal{H}\in \Sh(\mathcal{C}, \mathbf{Grp})$ be sheaves of groups such that for all $C \in \mathcal{C}$, $\mathcal{H}(C) \trianglelefteq \mathcal{G}(C)$ is a normal subgroup. Define the \textit{quotient sheaf} to be the sheafification:
\[
    \mathcal{G}/\mathcal{H} := L(\mathcal{G}/_{\mathrm{pre}}\mathcal{H}), \quad \text{where } \mathcal{G}/_{\mathrm{pre}}\mathcal{H}(C):= \mathcal{G}(C)/\mathcal{H}(C).
\]

\begin{enumerate}
    \item Show that any Lie group $G$ defines a sheaf of groups $\underline{G}\in \Sh(\mathrm{Mnfd}, \mathbf{Grp})$ by:
    \[
        \underline{G}(M):=\{f:M \to G \mid f \text{ is smooth}\},
    \]
    where $\mathrm{Mnfd}$ is the category of smooth manifolds with the coverage given by usual open covering.
    
    \item Let $\Omega^1_{\mathrm{cl}} \in \Sh(\mathrm{Mnfd}, \mathbf{Grp})$ be the sheaf of closed differential $1$-forms. Show that $\Omega^1_{\mathrm{cl}}$ is not concrete, i.e., there does not exist a Lie group $G$ such that $\underline{G} \cong \Omega^1_{\mathrm{cl}}$.
      (Hint: Compare $\underline{G}(\bR^0) $ and $ \Omega^1_{\mathrm{cl}}(\bR^0)$)
    \item Show that $\mathrm{d}: \underline{\mathbb{R}}\to\Omega^1_{\mathrm{cl}}$ is a morphism of sheaves in $\Sh(\mathrm{Mnfd}, \mathbf{Grp})$.
    
    \item Let $\mathbb{R}_{\mathrm{disc}}$ be the underlying discrete group of $\bR$ viewed as a discrete Lie group. Show that over Cartesian spaces $\mathbb{R}^i\in \Cart$ we have natural isomorphisms:
    \[
      \mathrm{d}: \underline{\mathbb{R}}/_{\mathrm{pre}}\underline{\mathbb{R}_{\mathrm{disc}}}(\mathbb{R}^i)\cong \Omega^1_{\mathrm{cl}}(\mathbb{R}^i),
    \]
    which induces the isomorphism of presheaf in $ \mathrm{PSh}(\Cart, \mathbf{Grp})$:
    \[
      (\underline{\mathbb{R}}/_{\mathrm{pre}}\underline{\mathbb{R}_{\mathrm{disc}}})|_{\Cart}\cong \Omega^1_{\mathrm{cl}}|_{\Cart}.
    \]
    (Hint: Use the Poincar\'e lemma.)
    
    \item Deduce that we have the isomorphism of sheaves in $\Sh(\mathrm{Mnfd}, \mathbf{Grp})$:
    \[
        \underline{\mathbb{R}}/\underline{\mathbb{R}_{\mathrm{disc}}}\cong \Omega^1_{\mathrm{cl}}.
    \]
  (Hint: Use that every open covering of a manifold can be refined to a good covering by Cartesian spaces.)
\end{enumerate}
This shows DeRham theorem of degree $1$: 
\[
  \mathrm{Coker}(C^{\infty}(M)\to \Omega^1_{\mathrm{cl}}(M))= \mathrm{Coker}( \underline{\mathbb{R}}\to \Omega^1_{\mathrm{cl}})(M)\cong \pi_0 \mathrm{B}\mathbb{R}_{\mathrm{disc}}(M)=\mathrm{H}^1_{\mathrm{Cech}}(M,\bR)
\]
\end{exercise}
