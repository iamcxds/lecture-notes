...
Give an object $X$ in a category, we can also consider the category of relative objects. In some sense those are ``families parameterized by $X$".
\begin{definition}[Slice Category]
  
 $\mathcal{C}$ be a category and let $X$ be an object of $\mathcal{C}$. The \emph{slice category} $\mathcal{C}/X$ is defined as follows:

\begin{itemize}
    \item \textbf{Objects:} Morphisms $f : A \to X$ in $\mathcal{C}$.
    \item \textbf{Morphisms:} A morphism from $f : A \to X$ to $g : B \to X$ is a morphism $h : A \to B$ in $\mathcal{C}$ such that $g \circ h = f$.
\end{itemize}
Dually, the \emph{coslice category} $X/\mathcal{C}$ has objects morphisms $f : X \to A$ and morphisms $h : A \to B$ satisfying $h \circ f = g$.
\end{definition}
\begin{exercise}
  \begin{enumerate}
    \item Assume $\mathcal{C}$ has fiber products. For $f: A \to X$ and $g: B \to X$ in $\mathcal{C}/X$, show that the product in $\mathcal{C}/X$ is the canonical morphism
    \[
        f \times_X g : A \times_X B \longrightarrow X.
    \]
    
    \item Let $A,S$ be sets and let $f:A\to S$ be a map of sets. Show that
    \[
        f^{-1}: S \longrightarrow \mathbf{Set}, \quad s \mapsto f^{-1}(s)
    \]
    defines a functor (where $S$ is viewed as a discrete category). Then prove the equivalence of categories:
    \[
        \mathbf{Set}/S \cong \mathrm{Fun}(S,\mathbf{Set}).
    \]

  In other words, $\Set/S$ is isomorphic to the category of $X$ indexed sets: objects are families of set $\{S_x\}_{x\in X}$, morphisms are families of maps $ \{\varphi_x: S_x \to T_x\}_{x\in X}$.
  \end{enumerate}
\end{exercise}
\section{Functor}
...
\begin{itemize}
  ...
  \[
    N(\cC)_n := \Fun([n], \cC)=\{ C_0 \xrightarrow{f_1} C_1  \xrightarrow{f_2} \cdots \xrightarrow{f_n} C_n\},
  \]
  viewing $[n]$ as a category via its poset structure.

  More concretely we have:
  \begin{itemize}
    \item $ N(\cC)_0 = \{C_0\}=\mathrm{Ob}(\cC)$,
    \item $ N(\cC)_1 = \{C_0\xrightarrow{f_1}C_1\}=\mathrm{Mor}(\cC) $, the face maps $d_0( C_0\xrightarrow{f_1}C_1)= C_0, d_1( C_0\xrightarrow{f_1}C_1)= C_1$, the degeneracy map $s_0(C_0)= [ C_0\xrightarrow{\mathrm{id}}C_0 ]$,
    \item $ N(\cC)_1 = \{C_0\xrightarrow{f_1}C_1\xrightarrow{f_2}C_2\}$, the face maps $d_0( C_0\xrightarrow{f_1}C_1\xrightarrow{f_2}C_2)= [C_0\xrightarrow{f_1}C_1], d_1( C_0\xrightarrow{f_1}C_1\xrightarrow{f_2}C_2)= [C_1\xrightarrow{f_2}C_2], d_2( C_0\xrightarrow{f_1}C_1\xrightarrow{f_2}C_2)= [C_0\xrightarrow{f_2\circ f_1}C_2]$, the degeneracy map $s_0(C_0\xrightarrow{f_1}C_1)= [ C_0\xrightarrow{\mathrm{id}}C_0 \xrightarrow{f_1}C_1], s_1(C_0\xrightarrow{f_1}C_1)=[C_0\xrightarrow{f_1}C_1\xrightarrow{\mathrm{id}}C_1]$,
...
\begin{exercise}
  \begin{enumerate}
    \item Let $M,N$ be smooth manifolds viewed as smooth sets. Show that a smooth map of manifold $f:M\to N$ induces a morphism of presheaves.
    \item  Show that every differential form $\omega \in \Omega^k(M)$ induces a morphism of smooth sets:
\[
\omega: M \to \Omega^k.
\]
...
\begin{exercise}
 ...
 Let $\Bool$ be the category consisting of two sets: $\bot = \emptyset$ and $\top = \{*\}$, with morphisms given by set maps. For any category $\cC$, define the category of 0-presheaves as:
\[
\PSh_0(\cC) := \mathrm{Fun}(\cC^\op, \Bool),
\]
i.e., contravariant functors from $\cC$ to $\Bool$.

\begin{enumerate}
  \item Show that the morphisms of $\Bool$ are:
    \[
      \Hom_{\Bool}(\bot,\bot)=\top, \Hom_{\Bool}(\top,\bot)=\bot, \Hom_{\Bool}(\bot,\top)=\top, \Hom_{\Bool}(\top,\top)=\top.
    \]
  \item Let $(P, \leq)$ be a thin category defined from a poset. For any object $a \in P$, show that the hom functor $h_a := \Hom_P(-, a) \in \PSh_0(P)$ is a 0-presheaf defined by:
    \[
    h_a(b) = 
    \begin{cases}
      \top, & \text{if } b \leq a, \\
  ...
  \item Consider the poset of rational numbers $(\bQ, \leq)$. Define a 0-presheaf $\sqrt{2} \in \PSh_0(\bQ)$ by:
    \[
    \sqrt{2}(q) = 
    \begin{cases}
      \top, & \text{if } q^2 \leq 2, \text{ or } q<0, \\
      \bot, & \text{otherwise}.
    \end{cases}
    \]
    Verify that $\sqrt{2}$ is indeed a 0-presheaf, and that $\sqrt{2} \neq h_q$ for any $q \in \bQ$.

  \item Show that $\PSh_0(\bQ)$ is isomorphic to the extended real line (as a thin category from poset):
    \[
    [-\infty, \infty] = \{-\infty\} \sqcup \bR \sqcup \{\infty\}.
    \]
\end{enumerate}
\end{exercise}
...
\begin{exercise}
 ...
 Let $X$ be a set, and let $\Sub(X)$ be the poset of subsets of $X$ ordered by inclusion $\subseteq$, viewed as a thin category.

\begin{enumerate}
  \item Let $(-)^c$ denote taking the complement. Show that:
    \[
    (-)^c : \Sub(X)^\op \to \Sub(X)
    \]
    is a functor.

  \item For $A, B \in \Sub(X)$, define:
    \[
    A \Rightarrow B := A^c \cup B \in \Sub(X).
    \]
    Show that for any $A, B, C \in \Sub(X)$, there is a bijection:
    \[
    \Hom_{\Sub(X)}(A \cap B, C) \cong \Hom_{\Sub(X)}(A, B \Rightarrow C).
    \]

  \item Let $f : X \to Y$ be a function between sets. Define:
    \[
    f^{-1} : \Sub(Y) \to \Sub(X), \quad f_! : \Sub(X) \to \Sub(Y), \quad f_!(A) := f(A).
    \]
    Verify that $f^{-1}$ and $f_!$ are functors, and show that for any $A \in \Sub(X)$, $B \in \Sub(Y)$, we have:
    \[
    \Hom_{\Sub(Y)}(f_!(A), B) \cong \Hom_{\Sub(X)}(A, f^{-1}(B)).
    \]

  \item Define:
    \[
    f_* : \Sub(X) \to \Sub(Y), \quad f_*(A) := f(A^c)^c.
    \]
    Verify that $f_*$ is a functor, and show that for any $A \in \Sub(Y)$, $B \in \Sub(X)$, we have:
    \[
    \Hom_{\Sub(X)}(f^{-1}(A), B) \cong \Hom_{\Sub(Y)}(A, f_*(B)).
    \]
\end{enumerate}
\end{exercise}
